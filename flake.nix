    ...
    let
      perSystemOutputs = flake-utils.lib.eachDefaultSystem (
        ...
        let
          ...
          testChecks = import ./lib/tests.nix { inherit pkgs lib; spg = libFor; };
        in
        {
          lib = libFor;
          checks = testChecks;
      ...
      extraLib = {
        mkSchemeWith = pkgs: (import ./lib { inherit pkgs; lib = pkgs.lib; }).mkScheme;
        mkExtraRepresentationsWith = pkgs: (import ./lib { inherit pkgs; lib = pkgs.lib; }).mkExtraRepresentations;
        generatorsWith = pkgs: (import ./lib { inherit pkgs; lib = pkgs.lib; }).generators;
